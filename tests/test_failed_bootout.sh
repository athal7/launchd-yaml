...
test_reload_bootout_fails_silent() (
	. "$TEST_DIR/lib/harness.sh"
	. "$TEST_DIR/lib/assert.sh"
	...
	export LAUNCHCTL_FAKE_LABEL_FOR_PLIST="$target"
	export LAUNCHCTL_FAKE_BOOTOUT_FAILS_SILENT=1

	run_launchd_yaml apply --file "$fixture"

	assert_exit_code 1 "$LAST_EXIT_CODE" "apply should fail when reload bootout fails silently"
	assert_contains "$LAST_STDERR" "ERROR: Failed to bootout demo" "stderr should report bootout failure"
	assert_not_contains "$LAST_STDERR" "launchctl bootout stderr:" "stderr should not show empty error output"
	assert_contains "$LAST_STDOUT" "1 failed" "summary should report failure"
)

test_prune_bootout_fails_silent() (
	. "$TEST_DIR/lib/harness.sh"
	. "$TEST_DIR/lib/assert.sh"
	...
	assert_exit_code 1 "$LAST_EXIT_CODE" "apply should fail when prune bootout fails silently"
	assert_contains "$LAST_STDERR" "ERROR: Failed to bootout pruned agent demo" "stderr should report prune bootout failure"
	assert_not_contains "$LAST_STDERR" "launchctl bootout stderr:" "stderr should not show empty error output"
	assert_contains "$LAST_STDOUT" "1 failed" "summary should report failure"
)

test_kickstart_fails_silent() (
	. "$TEST_DIR/lib/harness.sh"
	. "$TEST_DIR/lib/assert.sh"
	...
	export LAUNCHCTL_FAKE_KICKSTART_FAILS_SILENT=1

	run_launchd_yaml apply --file "$fixture" --self-agent selfhost

	assert_exit_code 1 "$LAST_EXIT_CODE" "apply should fail when kickstart fails silently"
	assert_contains "$LAST_STDERR" "ERROR: Failed to kickstart self-agent selfhost" "stderr should report kickstart failure"
	assert_not_contains "$LAST_STDERR" "launchctl kickstart stderr:" "stderr should not show empty error output"
)
